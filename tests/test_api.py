...
def test_read_single(model_fixtures, client):
    ...
    path = model_fixtures.path
    for create_opts in model_fixtures.create_opts:
        if model_fixtures.requires_relations:
        ...
        if model_fixtures.requires_relations:
            for relation in model_fixtures.requires_relations:  # there could be > 1 relation
                if isinstance(read_response[relation.field_name], list):  # maybe one-to-many
                    for resp in read_response[relation.field_name]:
                        compare_response(relation.create_opts, resp)  # TODO: Test len(list) > 1
                else:
